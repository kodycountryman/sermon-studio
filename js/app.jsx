...
function SettingsPanel({ currentTheme, onTheme, onClose, t, inline, preacherName, onPreacherName, defaultTrans, onDefaultTrans, fontSize, onFontSize }) {
  ...
  if (!folder) return (
    <div style={{...SS.flexColFull}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        />
        <MenuRow id="apikey" label="AI Provider" sub={getProvider()==="openai" ? "OpenAI GPT-4o" : "Claude Sonnet 4.6"} accent="#f59e0b"
  ...
  if (folder === "apikey") {
    ...
    const masked = keyVal ? keyVal.slice(0,8) + "..." + keyVal.slice(-4) : "";
    const placeholder = isOpenAI ? "sk-proj-..." : "sk-ant-...";
    const docsUrl = isOpenAI ? "platform.openai.com/api-keys" : "console.anthropic.com/keys";
    const switchProvider = (p) => {
      setProvider(p); setProviderState(p);
      setKeyVal(localStorage.getItem(p === "openai" ? "sermon_openai_key" : "sermon_anthropic_key") || "");
      setKeySaved(false);
    };
    return (
      <div className="drawer-enter" style={{...SS.flexColFull}}>
        <FolderHeader title="AI Provider"/>
        <div style={{flex:1,overflowY:"auto",padding:"20px 18px"}}>
          <SettingLabel>AI Provider</SettingLabel>
          <div style={{display:"flex",gap:8,marginBottom:20}}>
            {[{id:"anthropic",label:"Claude (Anthropic)",emoji:"🧠"},{id:"openai",label:"GPT-4o (OpenAI)",emoji:"⚡"}].map(({id,label,emoji})=>{
              const on = provider===id;
              return (
                <button key={id} onClick={()=>switchProvider(id)}
                  style={{flex:1,padding:"10px 8px",background:on?t.accentGrad:t.surface,border:`1.5px solid ${on?t.accent:t.surfaceBorder}`,borderRadius:9,cursor:"pointer",fontFamily:"Inter,sans-serif",fontSize:12,fontWeight:700,color:on?"#fff":t.text,transition:"all 0.15s"}}>
                  {emoji} {label}
                </button>
              );
            })}
          ...
          <SettingLabel>{isOpenAI ? "OpenAI" : "Anthropic"} API Key</SettingLabel>
          {keyVal && !keySaved ? (
            <div style={{background:t.surface,border:`1.5px solid ${t.surfaceBorder}`,borderRadius:8,padding:"10px 12px",marginBottom:10}}>
              <span style={{fontFamily:"monospace",fontSize:12,color:t.textMuted}}>{masked}</span>
            </div>
          ) : null}
          <input
            type="password"
            ...
            onFocus={e=>e.target.style.borderColor=t.accent}
            onBlur={e=>e.target.style.borderColor=t.inputBorder}
            style={{width:"100%",background:t.inputBg,border:`1.5px solid ${t.inputBorder}`,borderRadius:8,padding:"10px 12px",fontFamily:"monospace",fontSize:12,color:t.inputText,outline:"none",boxSizing:"border-box",transition:"border-color 0.15s"}}
            id="__apikey_input"
          />
          <button onClick={()=>{
            const v = document.getElementById("__apikey_input").value.trim();
            if (v) { setApiKey(v); setKeyVal(v); setKeySaved(true); setTimeout(()=>setKeySaved(false),2000); }
          }}
            style={{marginTop:10,width:"100%",padding:"10px 0",background:t.accentGrad,border:"none",borderRadius:8,fontFamily:"Inter,sans-serif",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer"}}>
            {keySaved ? "✓ Saved!" : "Save Key"}
          </button>
          <p style={{fontFamily:"Inter,sans-serif",fontSize:11,color:t.textMuted,margin:"12px 0 0",lineHeight:1.6}}>
            Stored locally on this device only. Get a key at <span style={{color:t.accent}}>{docsUrl}</span>.
          </p>
          {keyVal && (
            <button onClick={()=>{setApiKey("");setKeyVal("");document.getElementById("__apikey_input").value="";}}
              style={{marginTop:14,width:"100%",padding:"8px 0",background:"transparent",border:`1.5px solid #e74c3c55`,borderRadius:8,fontFamily:"Inter,sans-serif",fontSize:12,fontWeight:600,color:"#e74c3c",cursor:"pointer"}}>
              Remove Key
            </button>
          )}
        </div>
    ...
    <div className="drawer-enter" style={{...SS.flexColFull}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"32px 20px 20px"}}>
        ...
        {[
          ...
          {label:"AI Model",  value: getProvider()==="openai" ? "OpenAI GPT-4o" : "Claude Sonnet 4.6"},
...
// ─── UPLOAD PANEL ─────────────────────────────────────────────────────────────
function UploadPanel({ onClose, t, onVoiceUpdated, onStoriesFound }) {
  ...
  async function processFiles(fileList) {
    ...
    setPhase("uploading"); setProgress(10); setFiles(Array.from(fileList).map(f=>f.name));
    const texts = [];
    for (const file of fileList) {
      try {
        // For .pages files (zips), try extracting as text; fall back to filename only
        const text = await readFileText(file).catch(()=>"");
        // Strip binary garbage — keep only printable ASCII runs of 4+ chars
        const clean = (typeof text === "string" ? text : "")
          .replace(/[^\x20-\x7E\n\r\t]/g," ")
          .replace(/\s+/g," ")
          .trim();
        if (clean.length > 50) texts.push("=== " + file.name + " ===\n" + clean.slice(0,8000));
      } catch(e) {}
    }
    if (!texts.length) { setPhase("error"); return; }
    setProgress(35); setPhase("analyzing");

    const combined = texts.join("\n\n---\n\n").slice(0, 24000);
    try {
      const uploadSystem = `You are analyzing sermon manuscripts from a pastor to extract his unique voice fingerprint AND catalog his personal stories. Be specific. Use actual phrases and patterns you find in the text. Return ONLY valid JSON with this exact structure:
{
  "voiceSummary": "2-3 sentence plain text summary of voice",
  "openingStyle": "description",
  "humorStyle": "description",
  "crowdMoments": ["example 1","example 2"],
  "favoritePhrases": ["phrase 1","phrase 2","phrase 3"],
  "tensionStyle": "description",
  "gospelTurnStyle": "description",
  "closingStyle": "description",
  "keyMemory": ["personal fact 1","personal fact 2","personal fact 3"],
  "stories": [
    {
      "title": "Short title",
      "category": "funny or power",
      "tags": "tag1,tag2,tag3",
      "content": "Full story text as preached"
    }
  ]
}`;
      const rawText = await callAI({
        system: uploadSystem,
        messages: [{role:"user",content:`Analyze these sermon manuscripts and extract the voice fingerprint and all personal stories:\n\n${combined}`}],
        maxTokens: 4000,
      });
      setProgress(80);
      ...
      setResult(parsed);
      setMemory(parsed.keyMemory||[]);
      setProgress(100); setPhase("done");
      if (onStoriesFound && parsed.stories?.length) onStoriesFound(parsed.stories);
      if (onVoiceUpdated) onVoiceUpdated(parsed);
      try { await window.storage.set("sermon-voice-profile", JSON.stringify(parsed)); } catch(e){}
    } catch(e) {
    ...
    <div style={{...SS.flexColFull}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"20px 18px"}}>
        {phase==="idle" && (
          <>
            <div style={{textAlign:"center",padding:"20px 0 16px"}}>
              ...
              <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:13,lineHeight:1.6,margin:"0 0 20px"}}>Upload your past sermons and the AI will extract your unique voice, catalog your stories, and remember key details about you.</p>
            </div>
            <div
              ...
              <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:11,margin:0}}>TXT, DOCX, PDF, or any text-based format. 5 sermons minimum recommended.</p>
            </div>
            ...
            <div style={{background:`${t.accent}12`,border:`1px solid ${t.accent}25`,borderRadius:8,padding:"10px 12px"}}>
              <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:11,lineHeight:1.6,margin:0}}>
                What the AI learns: your opening style, humor patterns, favorite phrases, transition language, story structure, gospel turn language, and personal facts to remember.
              </p>
            </div>
          </>
        )}
        ...
        {(phase==="uploading"||phase==="analyzing") && (
          <div style={{textAlign:"center",padding:"40px 20px"}}>
            <div style={{fontSize:40,marginBottom:16}}>{phase==="uploading"?"📥":"🧠"}</div>
            <p style={{fontFamily:"Inter,sans-serif",color:t.text,fontSize:16,fontWeight:700,margin:"0 0 6px"}}>
              {phase==="uploading"?"Reading your sermons...":"Extracting your voice..."}
            </p>
            <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:12,margin:"0 0 20px",lineHeight:1.5}}>
              {phase==="analyzing"?"Finding your stories, phrases, and style patterns...":"Loading "+files.length+" file"+(files.length!==1?"s":"")+"..."}
            </p>
            <div style={{height:6,background:t.surface,borderRadius:10,overflow:"hidden",border:`1px solid ${t.surfaceBorder}`}}>
              <div style={{height:"100%",width:`${progress}%`,background:t.accentGrad,borderRadius:10,transition:"width 0.5s ease"}}/>
            </div>
            <p style={{fontFamily:"Inter,sans-serif",color:t.textFaint,fontSize:10,marginTop:6}}>{progress}%</p>
          </div>
        )}
        ...
        {phase==="done" && result && (
          <div>
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <p style={{fontFamily:"Inter,sans-serif",color:t.text,fontSize:16,fontWeight:700,margin:"0 0 4px"}}>Voice Profile Updated</p>
              <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:12}}>Found {result.stories?.length||0} stories. Your AI writing will now sound more like you.</p>
            </div>

            <div style={{background:t.surface,border:`1px solid ${t.panelBorder}`,borderRadius:8,padding:"14px 16px",marginBottom:12}}>
              <p style={{fontFamily:"Inter,sans-serif",color:t.accent,fontSize:10,textTransform:"uppercase",letterSpacing:"1.5px",fontWeight:700,margin:"0 0 6px"}}>Voice Summary</p>
              <p style={{fontFamily:"Inter,sans-serif",color:t.text,fontSize:12,lineHeight:1.7,margin:0,fontStyle:"italic"}}>{result.voiceSummary}</p>
            </div>

            {result.favoritePhrases?.length>0 && (
              <div style={{background:t.surface,border:`1px solid ${t.panelBorder}`,borderRadius:8,padding:"12px 14px",marginBottom:12}}>
                <p style={{fontFamily:"Inter,sans-serif",color:t.accent,fontSize:10,textTransform:"uppercase",letterSpacing:"1.5px",fontWeight:700,margin:"0 0 8px"}}>Your Phrases</p>
                <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                  {result.favoritePhrases.map((p,i)=>(
            ...
            {result.keyMemory?.length>0 && (
              <div style={{background:t.surface,border:`1px solid ${t.panelBorder}`,borderRadius:8,padding:"12px 14px",marginBottom:12}}>
                <p style={{fontFamily:"Inter,sans-serif",color:t.accent,fontSize:10,textTransform:"uppercase",letterSpacing:"1.5px",fontWeight:700,margin:"0 0 8px"}}>Key Memory ({result.stories?.length||0} stories saved to Story Bank)</p>
                {result.keyMemory.map((m,i)=>(
                  <p key={i} style={{margin:"0 0 4px",fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:11,lineHeight:1.5}}>• {m}</p>
                ))}
              </div>
            )}

            <button onClick={()=>{setPhase("idle");setFiles([]);setResult(null);}} style={{width:"100%",padding:"10px 0",background:t.surface,border:`1px solid ${t.panelBorder}`,borderRadius:7,color:t.textMuted,fontFamily:"Inter,sans-serif",fontSize:12,cursor:"pointer",marginTop:4}}>
              Upload More Sermons
            </button>
          </div>
        )}
        ...
        {phase==="error" && (
          <div style={{textAlign:"center",padding:"40px 20px"}}>
            ...
            <p style={{fontFamily:"Inter,sans-serif",color:t.text,fontSize:15,fontWeight:700,margin:"0 0 8px"}}>Couldn't read those files</p>
            <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:12,margin:"0 0 16px"}}>Try uploading plain text (.txt) or Word (.docx) files for best results.</p>
            <button onClick={()=>{setPhase("idle");setFiles([]);}} style={{padding:"8px 20px",background:t.accentGrad,border:"none",borderRadius:6,color:"#fff",fontFamily:"Inter,sans-serif",fontSize:12,fontWeight:700,cursor:"pointer"}}>Try Again</button>
          </div>
        )}
      </div>
    </div>
...
// ─── SOAP PANEL ──────────────────────────────────────────────────────────────
// ─── MAKE IT MINE PANEL ──────────────────────────────────────────────────────
function MakeMinePanel({ onClose, t, onLaunchCoach }) {
  ...
  const [pdfPhase, setPdfPhase] = useState("idle"); // idle | loading | done | error
...
Analyze the uploaded content and return a JSON object with this exact structure:
{
  ...
  async function processPdf(file) {
    setPdfFileName(file.name); setPdfPhase("loading"); setPdfHtml(""); setPdfErr(""); setCopied(false);
    try {
      ...
      function normalizeColor(c) {
        ...
        // PDF.js returns colors as 0–255 integers for annotations
        const max = Math.max(c[0], c[1], c[2]);
        // If all values ≤ 1 it's a 0–1 float range, scale up
        return max <= 1 ? c.map(v => Math.round(v * 255)) : [c[0], c[1], c[2]];
      }

      function colorToStyle(rawColor) {
        const [r, g, b] = normalizeColor(rawColor);
        if (g > r && g > b && g > 60) {
          // Green → yellow highlight + bold + underlined (black text)
        ...
        return null; // unknown color → treat as un-highlighted
      }
      ...
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        for (const yk of sortedYs) {
          ...
          for (const item of lineItems) {
            if (!item.str) continue;
            const [,,,,ix,iy] = item.transform;
            const hlColor = getHighlightColor(ix, iy, item.width);
            const escaped = escHtml(item.str);
            if (hlColor) {
              const style = colorToStyle(hlColor);
              lineHtml += style
                ? `<span style="${style}">${escaped}</span>`
                : `<span style="font-weight:400;">${escaped}</span>`;
            } else {
              lineHtml += `<span style="font-weight:400;">${escaped}</span>`;
            }
          }
          const plainCheck = lineHtml.replace(/<[^>]+>/g, "").trim();
          if (plainCheck) {
            html += `<div style="margin:0;padding:0;line-height:1.7;font-family:Arial,sans-serif;font-size:12pt;color:#111;">${lineHtml}</div>`;
          }
        }

        if (pageNum < pdf.numPages) {
          html += `<div style="margin:14px 0;border-top:1px solid #ddd;padding-top:14px;font-family:Arial,sans-serif;font-size:9pt;color:#aaa;text-align:center;">— page ${pageNum} —</div>`;
        }
      }
          ...
          <>
            {pdfPhase === "idle" && (
              <>
                <div
                  ...
                  onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=t.accent;}}
                  onDragLeave={e=>e.currentTarget.style.borderColor=t.surfaceBorder}
                  onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f&&f.type==="application/pdf")processPdf(f);}}
                  style={{border:`2px dashed ${t.surfaceBorder}`,borderRadius:12,padding:"36px 20px",textAlign:"center",cursor:"pointer",transition:"border-color 0.2s",marginBottom:16}}>
            ...
            {pdfPhase === "loading" && (
              <div style={{textAlign:"center",padding:"50px 20px"}}>
                ...
                <p style={{fontFamily:"Inter,sans-serif",color:t.text,fontSize:15,fontWeight:700,margin:"0 0 6px"}}>Reading highlights...</p>
                <p style={{fontFamily:"Inter,sans-serif",color:t.textMuted,fontSize:12,margin:0}}>{pdfFileName}</p>
              </div>
            )}
...
function CoachView({ project, onExit, t }) {
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      window.cloudProjects.save({ ...project, content, title, coachMessages: messages }).catch(()=>{});
    }, 2000);
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const reply = await callAI({ system: systemWithContent, messages: aiMessages, maxTokens: 4000 });
  ...
  return (
    <div style={{display:"flex", flex:1, overflow:"hidden"}}>
      {/* ── LEFT: Sermon Editor ────────────────────────────────────────────── */}
      <div style={{flex:1, display:"flex", flexDirection:"column", borderRight:`1px solid ${t.panelBorder}`}}>
        {/* Editor header */}
        <div style={{padding:"12px 20px", borderBottom:`1px solid ${t.panelBorder}`, display:"flex", alignItems:"center", gap:10, flexShrink:0}}>
          <button onClick={onExit}
            style={{padding:"6px 14px", borderRadius:20, border:`1px solid ${t.surfaceBorder}`, background:"transparent", color:t.textMuted, fontFamily:"Inter,sans-serif", fontSize:11, fontWeight:600, cursor:"pointer"}}>
            Exit Coach
          </button>
          <input
            value={title}
            ...
            style={{flex:1, background:"transparent", border:"none", color:t.text, fontFamily:"Inter,sans-serif", fontSize:16, fontWeight:700, outline:"none"}}
          />
          <span style={{fontFamily:"Inter,sans-serif", fontSize:10, color:t.textMuted, letterSpacing:1}}>AUTO-SAVED</span>
        </div>
        ...
        <textarea
          ...
          placeholder="Type or paste your sermon content here...&#10;&#10;The AI coach can see everything you write and will reference specific parts when giving feedback."
          style={{flex:1, width:"100%", padding:"24px", background:t.bg, border:"none", color:t.text, fontFamily:"Arial,sans-serif", fontSize:"12pt", lineHeight:1.8, resize:"none", outline:"none", boxSizing:"border-box"}}
        />
      </div>

      {/* ── RIGHT: AI Coach Chat ──────────────────────────────────────────── */}
      <div style={{width:420, minWidth:320, display:"flex", flexDirection:"column", background:t.panelBg}}>
        {/* Coach header */}
        <div style={{padding:"12px 16px", borderBottom:`1px solid ${t.panelBorder}`, flexShrink:0}}>
          <h3 style={{margin:0, fontFamily:"Inter,sans-serif", color:t.text, fontSize:14, fontWeight:700}}>AI Sermon Coach</h3>
          <p style={{margin:"2px 0 0", fontFamily:"Inter,sans-serif", fontSize:10, color:t.textMuted, letterSpacing:"0.5px"}}>Andy Stanley + Rich Wilkerson Jr + Furtick + Lentz + Groeschel</p>
        </div>

        {/* Hot buttons */}
        <div style={{padding:"10px 12px", borderBottom:`1px solid ${t.panelBorder}`, display:"flex", flexWrap:"wrap", gap:5, flexShrink:0}}>
          {COACH_HOT_BUTTONS.map(btn => (
            <button key={btn.id}
              onClick={() => handleHotButton(btn)}
              disabled={sending}
              style={{padding:"5px 10px", borderRadius:14, border:`1px solid ${t.surfaceBorder}`, background:t.surface, color:t.textMuted, fontFamily:"Inter,sans-serif", fontSize:11, fontWeight:600, cursor:sending?"not-allowed":"pointer", opacity:sending?0.5:1, transition:"all 0.15s", whiteSpace:"nowrap"}}>
              {btn.emoji} {btn.label}
            </button>
          ))}
        </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try { const r = await window.storage.get("sermon-voice-profile"); if (r?.value) setVoiceProfile(JSON.parse(r.value)); } catch(e) {}
    })();
    (async () => {
  ...
  async function generate(force=false) {
    ...
    try {
      // Build closing moment instruction
      ...
      // Build tune instructions (voice blend, venue, audience, outline method)
      const tuneExtra = buildTuneInstructions(tuneSettings, modeId);
      const sys = (mode.isEditor ? buildSystemWithLength(mode.system, length) : mode.system) + closingExtra + tuneExtra;
      ...
      const userContent = mode.isEditor
        ...
        : mode.userMsg(input);
      const text = await callAI({ system:sys, messages:[{role:"user",content:userContent}], maxTokens:16000 }) || "No response received.";
      setOutput(text); setLastInput(input); setActiveVersion(null);
      // Version stack: keep last 3, push new to front
      setVersionStack(prev => {
        const entry = { output:text, input, ts:Date.now(), label:"v"+(prev.length+1) };
        const updated = [entry, ...prev].slice(0,3);
        // Re-label oldest first: v1, v2, v3
  ...
  async function expandToManuscript() {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const sys = buildSystemWithLength(msMode.system, length) + closingExtra + tuneExtra;
        const text = await callAI({ system:sys, messages:[{role:"user",content:`Write a full word-for-word sermon manuscript from this outline:\n\n${outlineText}`}], maxTokens:16000 }) || "No response received.";
  ...
  const css = useMemo(() => `
    ...
    *{box-sizing:border-box;}
    ...
    @media(max-width:600px){
      ...
      .mode-pill-wrap{padding:4px!important;gap:3px!important;border-radius:14px!important;}
      .mode-pill-btn{flex-direction:column!important;gap:2px!important;padding:7px 2px!important;border-radius:10px!important;font-size:10px!important;letter-spacing:0!important;}
      .mode-pill-btn .mode-icon svg{width:16px!important;height:16px!important;}
      .app-ta{font-size:14px!important;padding:14px!important;}
  ...
  return (
    <div style={{display:"flex", height:"100vh", overflow:"hidden", background:t.bg, color:t.text}}>
      ...
      <div className={`side-panel ${sidePanel ? "" : "collapsed"}`} style={{width: sidePanel ? SIDEBAR_W : 0, minWidth:0}}>
        ...
        {sidePanel === "makemine" && (
          <MakeMinePanel onClose={()=>setSidePanel(null)} t={t} onLaunchCoach={launchCoach}/>
        )}
      </div>

      {/* ── RIGHT: main content area (or Coach View) ── */}
      ...
      <div style={{flex:1, overflowY:"auto", display:"flex", flexDirection:"column"}}>
        <div className="main-pad" style={{maxWidth:900, margin:"0 auto", padding:"52px 32px 80px", width:"100%"}}>
          ...
          <button className="gbtn" onClick={generate} disabled={loading||!input.trim()} style={{position:"relative",overflow:"hidden"}}>
            {loading ? (
